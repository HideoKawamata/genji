...
**Genji Mirror** utilizes a modern decoupled architecture where a React-based frontend interacts with a robust Python backend. This separation of concerns allows for a highly interactive user experience while securely managing complex AI processes and vector database operations on the backend.

### 1. Frontend Architecture (Next.js)
The frontend is built with **Next.js 16.2 (App Router)** and **React 19**, prioritizing performance, SEO, and developer experience.
- **Styling & UI**: Tailwind CSS is used for utility-first styling, enabling a responsive and cohesive premium design. Framer Motion handles fluid, Heian-inspired micro-animations and page transitions to enrich the user experience.
- **State Management**: React Hooks manage local states (e.g., user input, loading status, selected avatars), ensuring the UI instantly reacts to user interactions.
- **Integration**: The frontend communicates securely with the backend via RESTful APIs (`/api/consult` for AI generation, `/api/auth/*` for user management), using JWT tokens for authenticated routes.

### 2. Backend Architecture (FastAPI)
The backend is a high-performance **FastAPI** Python application that serves as the orchestration layer for all core functionalities.
- **Authentication System**: Utilizes PyJWT to handle token-based authentication. User credentials and avatars are securely stored in a local **SQLite** database (`users.db`), with passwords hashed using bcrypt (PBKDF2-HMAC-SHA256).
- **Asynchronous Processing**: FastAPI's native async capabilities, combined with Python's `asyncio.to_thread()`, are heavily utilized to prevent blocking the event loop during heavy I/O operations, such as querying GCP services and ChromaDB.
- **Vector Search Engine**: **ChromaDB** acts as the localized vector database, storing and retrieving thousands of embedded text chunks from *The Tale of Genji*. It provides the crucial retrieval backbone for the RAG system.

### 3. AI & ML Integration (Google Cloud Vertex AI)
The system deeply integrates with **Google Cloud Vertex AI** to handle all generative and embedding tasks.
- **Embeddings**: `text-multilingual-embedding-002` translates Japanese text into 768-dimensional vectors, enabling semantic similarity searches across the classic literature.
- **Text Generation**: `gemini-2.5-flash` acts as the empathetic agent, capable of understanding modern concerns, rewriting queries, processing retrieved Heian-period context, and generating empathetic responses along with classical Waka poetry.
- **Image Generation**: `imagen-3.0-generate-002` dynamically generates stunning 16:9 Heian scroll-style illustrations based on English prompts generated by Gemini, creating a multi-modal user experience.
